  ...
  return (
    <div style={{ fontFamily: "'Nunito',sans-serif", background: "#FDF6FB", minHeight: "100vh" }}>
      <Hdr
        ...
        right={cart.length > 0
          ? <span style={{ background: pk, color: "#fff", borderRadius: 20, padding: "2px 10px", fontSize: 13, fontWeight: 800 }}>{cart.length}</span>
          : null
        }
      />
      ...
      <div style={{ padding: 16, maxWidth: 500, margin: "0 auto" }}>
        ...
        {cart.length === 0 && !addingIt && (
          <div style={{ textAlign: "center", padding: "40px 0", color: "#888" }}>
            ...
            <Btn ch="← Voltar" onClick={() => setScreen("pedido-info")} />
          </div>
        )}
        ...
        {cart.map((it, idx) => {
          ...
          const prod  = TIPOS?.find(t => t.id === it.tipo) ?? null;
          ...
          return editIdx === idx ? (
            ...
            <div key={idx} className="hov" style={{ background: "#fff", border: `1px solid ${pkL}`, borderLeft: `4px solid ${pk}`, borderRadius: 14, padding: 14, marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div style={{ flex: 1 }}>
                  {prod?.fotoUrl && !isU && (
                    <img src={prod.fotoUrl} style={{ width: 60, height: 40, objectFit: "cover", borderRadius: 8, marginBottom: 6 }} onError={e => { e.target.style.display = "none"; }} alt="" />
                  )}
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "flex-start" }}>
                  {!isU && (
                    <button onClick={() => setEditIdx(idx)} style={{ background: pkL, color: pk, border: "none", borderRadius: 8, padding: "5px 10px", cursor: "pointer" }}>✏️</button>
                  )}
        ...
        {addingIt && editIdx === null ? (
          ...
          <button
            ...
            style={{ width: "100%", border: `2px dashed ${pk}`, borderRadius: 14, padding: 14, background: "none", color: pk, fontSize: 14, fontWeight: 800, marginBottom: 14, cursor: "pointer" }}
          >
            + Adicionar item
          </button>
        )}
            ...
            <div style={{ background: "#fff", border: `2px solid ${pkL}`, borderRadius: 14, padding: 14, marginBottom: 14 }}>
              {taxaReg > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#888" }}>
                  <span>Taxa de entrega</span><span>{fmtR(taxaReg)}</span>
                </div>
              )}
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, fontWeight: 900 }}>
                <span>Total</span><span style={{ color: pk }}>{fmtR(cartFinal)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#BE185D", fontWeight: 700 }}>
                <span>Caução (50%)</span><span>{fmtR(cartCaucao)}</span>
              </div>
              {cartSaldo > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, fontWeight: 900, color: "#B45309", marginTop: 6, background: "#FFFBEB", padding: "4px 8px", borderRadius: 6 }}>
                  <span>💳 SALDO A COBRAR</span><span>{fmtR(cartSaldo)}</span>
                </div>
              )}
            </div>
