...
export function PedidoDoneScreen({
  caucao, wppUrl, onClientArea, onHome,
  hasClient, total, saldo,
}) {
  const [copied, setCopied] = useState(false);

  const mensagem = total
    ? `Olá, seu pedido ficou no valor total de ${fmtR(total)}. Foi pago ${fmtR(caucao)} de entrada. SALDO A COBRAR: ${fmtR(saldo)}.`
    : "";
      ...
      <div style={{ padding: "24px 18px", maxWidth: 480, margin: "0 auto", textAlign: "center" }}>
        ...
        {caucao > 0 && (
          <div style={{ background: "#F0FDF4", border: "1.5px solid #BBF7D0", borderRadius: 14, padding: 16, marginBottom: 14, textAlign: "left" }}>
            ...
            {saldo > 0 && (
              <div style={{ fontWeight: 900, color: "#B45309", textTransform: "uppercase" }}>
                💳 SALDO A COBRAR: {fmtR(saldo)}
              </div>
            )}
        ...
        {/* Mensagem de saldo */}
        {mensagem && (
          <div style={{ background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: 10, padding: 12, marginBottom: 14, textAlign: "left", fontWeight: 700, color: "#B45309" }}>
            {mensagem}
          </div>
        )}
        ...
        {hasClient && (
          <Btn ch="Ver meus pedidos" st={{ background: pkD, marginBottom: 8 }} onClick={onClientArea} />
        )}
